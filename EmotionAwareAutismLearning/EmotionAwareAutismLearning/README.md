...
```text
EmotionAwareAutismLearning/
├── app.py                   # Flask application entry point
├── config.py                # Configuration (paths, email settings, etc.)
├── requirements.txt
├── .gitignore
├── models/
│   ├── __init__.py
│   └── emotion_model.py     # Placeholder InceptionV3-based model wrapper
├── services/
│   ├── __init__.py
│   ├── camera_service.py    # Webcam capture utilities
│   ├── audio_service.py     # Audio upload & features stub
│   ├── learning_service.py  # Tracks child attempts & learning logic
│   ├── report_service.py    # Report generation
│   └── email_service.py     # Email sending helper
├── templates/
│   ├── base.html
│   ├── index.html           # Parent dashboard / landing
│   ├── child_learning.html  # Child emotion learning interface
│   ├── upload_audio.html    # Audio irritation test page
│   └── report.html          # Simple report view
├── static/
│   ├── css/
│   │   └── styles.css
│   └── js/
│       └── main.js
├── data/
│   ├── child_responses.csv  # Simple CSV log (created at runtime)
│   └── irritation_log.csv   # Simple CSV log (created at runtime)
└── tests/
    └── test_smoke.py
```
...
- `models/` – Place your **trained InceptionV3 model** here (e.g. `emotion_model.h5`) and load it inside `emotion_model.py`.
- `data/` – Application creates CSV logs of:
  - child emotion learning attempts
  - audio irritation detection sessions

## How to Integrate Your Real Model

1. Train your InceptionV3 model separately using FER2013 or any other dataset.
